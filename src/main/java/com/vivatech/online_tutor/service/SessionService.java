...
import com.vivatech.online_tutor.helper.CustomUtils;
...
import com.vivatech.online_tutor.repository.TutorSessionRepository;
...
public class SessionService {
    ...
    public SessionResponseDTO getSessionById(Integer id) {
        ...
        TutorSession session = tutorSessionRepository.findById(id)
                .orElseThrow(() -> new CustomExceptionHandler("Session not found with ID: " + id));
    ...
    public List<SessionResponseDTO> getAllSessions(String userName, LocalDate viewDate) {
        ...
        List<TutorSession> sessions = tutorSessionRepository
                .findByCreatedByAndSessionEndDateGreaterThanEqual(userRepository.findByUsername(userName)
                        .orElseThrow(() -> new CustomExceptionHandler("User not found")), today);
    ...
    public void deleteSession(Integer id) {
        ...
        if (!tutorSessionRepository.existsById(id)) {
            throw new CustomExceptionHandler("Session not found with ID: " + id);
        }
    ...
    private TutorSession mapToEntity(SessionRequestDTO dto) {
        User createdBy = userRepository.findByUsername(dto.getCreatedBy()).orElseThrow(() -> new CustomExceptionHandler("User not found"));
        ...
        if (!session.getIsRecurring()) {
            if (dto.getUpcomingDates().isEmpty()) throw new CustomExceptionHandler("Upcoming dates are required for non-recurring sessions");
