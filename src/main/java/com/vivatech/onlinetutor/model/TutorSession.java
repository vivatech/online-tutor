    ...
    public List<LocalDate> calculateUpcomingDates() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (recurrenceFrequency == RecurrenceFrequency.WEEKLY) {
                ...
                if (dayOfWeek.stream().anyMatch(ele -> ele.equalsIgnoreCase(finalNextDate.getDayOfWeek().toString()))) {
                    System.out.println("Adding dates: " + finalNextDate + " which is: " + finalNextDate.getDayOfWeek().toString());
                    dates.add(finalNextDate);
                }
            }
        }
